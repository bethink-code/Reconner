      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl bg-section p-4">
          ...
          <div>
            ...
            <p className="mt-1 text-2xl font-bold tabular-nums text-[#1A1200]">{formatRand(sales.total.amount)}</p>
            <p className="text-xs text-muted-foreground">{sales.total.count} sales</p>
          </div>
          <div className="mt-3 border-t border-border/50 pt-2 space-y-0.5">
            <Row label="Card" amount={sales.card.amount} count={sales.card.count} />
            <Row label="Cash" amount={sales.cash.amount} count={sales.cash.count} />
            {sales.other.count > 0 && (
              <Row label="Other tenders" amount={sales.other.amount} count={sales.other.count} />
            )}
          </div>
        </div>
      </div>
