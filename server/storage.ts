...
export class DatabaseStorage implements IStorage {
  ...
  async upsertUser(userData: UpsertUser): Promise<User> {
    // First check if user exists by email (to handle existing users with different IDs)
  ...
  async getTransactionsByPeriodPaginated(
    ...
    if (periodDates) {
      ...
      // All display/review uses period dates — the wider date window is for matching only
    ...
    const result = await pool.query(`
      ...
      tx_stats AS (
        SELECT
          ...
          -- Bank counts scoped strictly to period dates (settlement lag is a matching
          -- technique only — out-of-period bank never inflates period totals, even if matched)
          ...
          -- Matched bank scoped to period dates (out-of-period matched bank is a lag-day
          -- match and belongs to a different period's reporting, not this one)
          ...
          COALESCE(SUM(CASE WHEN source_type LIKE 'bank%' AND match_status = 'excluded'
                     AND transaction_date >= dw.min_date AND transaction_date <= dw.max_date THEN amount::numeric ELSE 0 END), 0) as excluded_bank_amount,
          COUNT(CASE WHEN source_type LIKE 'bank%' AND match_status = 'resolved'
                     AND transaction_date >= dw.min_date AND transaction_date <= dw.max_date THEN 1 END) as resolved_bank_transactions,
  ...
  private async getPerBankBreakdown(periodId: string): Promise<PerBankBreakdown[]> {
    const result = await pool.query(`
      ...
      JOIN reconciliation_periods rp ON rp.id = $1
      WHERE t.period_id = $1 AND t.source_type LIKE 'bank%'
        AND t.transaction_date >= rp.start_date AND t.transaction_date <= rp.end_date
